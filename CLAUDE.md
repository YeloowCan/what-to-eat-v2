...
- **Display-info detail fallback**: `PoiDisplayPort.get(poiId)` serves the around-search cache (populated during `find()`); on cache miss it returns `null` (cuisine placeholder) rather than calling a POI detail API — Amap v3 has no detail-by-id endpoint and v5 `/place/detail` needs a v5-enabled key (unverified). Harmless for the reveal card (its poiId always hits the cache); the miss path only matters for history-list photos (Out of Scope). See ADR-0005.
...
Conventional commits with scopes: `feat(ui):`, `feat(engine):`, `chore(deps):`, `docs:`. Work is sliced into tickets tracked under `.scratch/mvp/issues/` (engine MVP, issues 01–06) and `.scratch/ui-redesign/issues/` (real-wheel + fortune redesign, tickets 01–06), and `.scratch/display-info/issues/` (card display info, tickets 01–03); commits reference the ticket as `(ticket NN)`. Specs: `.scratch/mvp/PRD.md` (engine), `.scratch/ui-redesign/PRD.md` (UI), `.scratch/display-info/PRD.md` (display info). Architecture decisions in `docs/adr/0001…0005`. `.scratch/` is tracked in git on purpose — it holds the PRD and issues.
